...
public class SpaceExplorer extends Thread {
	...
	public void run() {
		Message mod;
		while (true) {
			mod = channel.getMessageHeadQuarterChannel();
			if (mod == null) {
				break;
			}
			if (mod.getData() == HeadQuarter.EXIT) {
				break;
			}
			if (discovered.contains(mod.getCurrentSolarSystem())) {
				continue;
			}
			int parent = mod.getParentSolarSystem();
			if (parent == -1) {
				parent = mod.getCurrentSolarSystem();
			}
			String data = encryptMultipleTimes(mod.getData(), hashCount);
			Message solution = new Message(parent, mod.getCurrentSolarSystem(), data);
			channel.putMessageSpaceExplorerChannel(solution);
			synchronized(channel) {
				discovered.add(mod.getCurrentSolarSystem());
			}
		}
	}
